...
A single Go binary on your host serves the web admin (`/admin/`) and a REST + WebSocket API (`/api/v1/*`). Clients drive sessions over HTTP/WS; the session manager spawns each AI CLI in its own PTY; the memory layer keeps shared state in Postgres with vector embeddings from your own provider. Everything runs on your network — no inference outside your control.

> **Note:** opendray shares process state (`~/.claude`, ssh-agent, project files) with the CLIs it spawns, which is incompatible with production container isolation — Docker is not a supported deploy path for v2.x. Run it on a host (bare metal, VM, or LXC).
...
**macOS (Mac mini / Studio as home server)** uses a LaunchDaemon, and any other supervisor (OpenRC, FreeBSD `rc.d`) works with the plain binary. Full walkthroughs — secrets file, launchd plist, updates — in [docs/install-binary.md](docs/install-binary.md).
