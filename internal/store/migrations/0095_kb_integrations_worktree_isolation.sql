-- 0095 — kb_integrations refresh: re-seed the Third-Party Integration
-- Guide after session creation gained the optional `isolation` field.
-- POST /api/v1/sessions now accepts isolation="worktree" (spawn the CLI
-- in a private git worktree on branch opendray/<session-id>), the
-- session JSON gained work_dir / worktree_branch, and GET
-- /api/v1/git/info is the repo probe for deciding whether isolation is
-- offerable for a cwd. The guide's "Create a session" section documents
-- the field, its git-main-checkout precondition, and the logical-cwd /
-- physical-work_dir identity split.
--
-- Why a new migration instead of editing 0093: the runner records
-- applied versions by filename and never re-applies one, so the live KB
-- page stays frozen at the previous text on every DB that already ran
-- it. This migration force-updates the page content (DO UPDATE),
-- keeping updated_by='operator' so the AI KB drafter treats it as
-- human-locked.
--
