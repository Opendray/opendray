...
carried 0025's `CHECK (kind IN ('goal','plan'))`, so every KB-page /
...
- `integrations.memory_policy` (none|quarantine|full), default
  ...
  agents, so quarantining it would break their memory pipeline).
...
- `/cortex` is the flywheel home: per-rung cards with live counts
  (pending proposals split project-vs-global, quarantine badge),
  active-project jump list.
- `/cortex/project` is the project workspace: tabs ARE the blueprint;
...
- Old API routes (`/project-docs*`, `/memory*`, `/knowledge/*`) stay
  mounted, and integrations and the mobile app keep working unchanged.
...
- Migrations 0044–0048 run on next binary start; committing them is
  safe, and nothing reaches the live service until the operator triggers
...
1. The proposals-CHECK fix may release a backlog of KB update
   proposals on the first consolidation cycle, so watch the Inbox.
...
3. Garbage cwds in project_docs each received 5 seeded blueprint
   sections, which is cosmetic; archive or reset those projects as desired.
