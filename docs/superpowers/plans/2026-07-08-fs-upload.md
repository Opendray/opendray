...
Expected: build succeeds (tsc has no errors), lint clean. If tsc complains that `webkitGetAsEntry` is missing on `DataTransferItem`, it is part of `lib.dom.d.ts`, so confirm `tsconfig` includes `"DOM"` in `lib` (it does via Vite's default); no extra typings needed.
