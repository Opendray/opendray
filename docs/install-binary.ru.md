...
Бинарник и есть *весь* шлюз целиком: React-админка встроена в него, поэтому
...
|---|---|---|---|
| `OPENDRAY_DATABASE_URL` | **да** | нет | DSN для Postgres |
| `OPENDRAY_ADMIN_PASSWORD` | рекомендуется | нет | Пароль для веб/мобильной админки |
...
при краше. Об этом далее.

---

## Запуск как сервис

`opendray serve` это именно та команда, которую должен вызывать start-команда юнита
