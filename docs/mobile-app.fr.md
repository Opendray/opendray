...
hôte gateway, et le téléphone ne fait que s'y rattacher.
...
4. **Bundle Identifier** : il est livré en `io.opendray.opendray`. Avec un
   Apple ID gratuit, cet ID exact peut déjà être pris côté Apple, donc
...
| La connexion marche mais le terminal Sessions ne se connecte jamais | Le reverse proxy laisse tomber l'upgrade WebSocket | Ajoute les headers WS, voir [operator-guide §Topology](operator-guide.md#topology) |
