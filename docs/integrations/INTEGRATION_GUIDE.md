...
- **Landed since the first cut of this guide (now current, no longer forthcoming):** the provider-agnostic `permission_mode` field replacing the old `bypass_permissions` boolean ([§6](#6-the-unified-spawn-profile--configuring-what-runs)); the per-principal `integration:<id>` memory zone ([§8](#8-memory--privacy)); and scope enforcement on the provider *mutation* routes (`providers:write` / `providers:update`, [§5](#5-scopes--authorization)). Build against `permission_mode` + `memory_policy=none`.
