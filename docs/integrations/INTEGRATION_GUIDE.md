...
Every session your integration creates is stamped server-side with `origin=integration` and your `integration_id`. **These fields are derived from your authenticated principal, NEVER from the request body**, so you cannot spoof them. This gives three guarantees:
...
|---|---|---|---|
| `name` | **MUST** | n/a | Unique display label. DB-unique. A `409` may mean a name **or** prefix collision (see edge cases). |
...
> **The default `memory_policy` is `none`.** (An internal code comment says "quarantine", but it is stale and wrong; the registration path sets `none`.)
...
System integrations (`is_system: true`, e.g. the opendray-memory MCP bridge) **cannot** be deleted or rotated by the operator (`403 ErrSystemIntegration`), because destroying their key would orphan running sessions whose `mcp.json` references it.
...
**WebSocket** (browsers can't set custom WS headers, so use the query param):
```
...
> Note the spelling: the (unenforced) read scope is `provider:read` (singular), while the (enforced) mutation scopes are `providers:write` / `providers:update` (plural). They are distinct strings, so grant exactly what the code checks.
...
- There is **no hierarchy**: `memory:*` does NOT expand to `memory:read`/`memory:write`, so grant each explicitly.
...
- **NEVER** hand-build per-CLI flags (`--mcp-config`, `--append-system-prompt`, `--rules`, `--dangerously-skip-permissions`, `--yolo`, …) into the request `args`. That hard-locks you to one CLI and breaks the moment the operator switches the default provider.
...
Grok specifics: MCP servers merge into the project-scoped `<cwd>/.grok/config.toml` (only opendray-managed entries are touched; the operator's global `~/.grok/config.toml` is never modified). Your `system_prompt` is delivered through grok's `--rules` flag; grok accepts that flag at most once per invocation, so opendray coalesces every system-prompt fragment for the spawn into a single `--rules` value. Grok has no unattended-permissions CLI flag, so `permission_mode: bypass` adds no flag on grok spawns; use the provider's own `bypassPermissions` setting if you need unattended runs there.
...
- `isolation` (optional): `"worktree"` spawns the CLI in a **private git worktree** on branch `opendray/<session-id>` instead of directly in `cwd`, so concurrent sessions on the same project cannot clobber each other's files. Requires `cwd` to be inside a git repository's main checkout (`400` otherwise; probe first with `GET /api/v1/git/info?path=<cwd>` → `{is_repo, linked_worktree, repo_root, branch}`). The session's memory scope, project docs, and Cortex identity stay keyed by the logical `cwd`; the response carries the physical path in `work_dir` and the managed branch in `worktree_branch`. Changes merge back through the normal branch → PR flow; untracked files (`.env`, `node_modules`) are **not** copied into the worktree. Omit the field for the default run-in-cwd behaviour.
- Response `201` with the session (`id`, `state`, `pid`, `origin: "integration"`, `integration_id`, and `work_dir`/`worktree_branch` when isolated).
...
- `POST /start` re-spawns under the original provider/cwd/args/account. For a terminal (ended/stopped) row this is a **fresh PTY, so prior in-process context is lost** (the row/history is preserved). Returns `200` + session. It does **not** resume a live process.
...
**Supported engines:** `postgres`, `mysql`, `mariadb`, `sqlite` (the connection's `driver` field). MySQL and MariaDB share one implementation and use host/port/username/password like PostgreSQL (a MySQL "schema" is a database). **SQLite is a file-path connection**: `db_name` holds the path (no host/port/username), the path is fenced to the connection's project `cwd` (a path escaping it via `../` or a symlink is rejected at open time), and extension loading is disabled. All engines share the same REST surface, read-only fence and read/write scopes below; only the connection form differs.
...
- **The auto-attached `opendray-dbtool` MCP is not given to `origin=integration` sessions**, the same isolation rule as memory. Your integration reaches the Database tool through its own scoped key over REST, not through a spawned session's ambient tools, and only for connections registered under a project it is allowed to see.
...
A per-integration summary endpoint (`/integrations/_calls/summary`) is a **TODO**, not yet wired, so do not call it.
...
- **Memory partitioning is by principal.** Capture for a `quarantine`/`full` integration lands in its own `integration:<id>` zone, so memory does not leak across integrations even on a shared cwd ([§8](#8-memory--privacy)). File-level isolation is still cwd/OS-based (the point above), so a distinct cwd remains good hygiene.
...
| `409` | Registration name/prefix/reserved-prefix conflict; driving an ended session (`ErrAlreadyEnded`); provider unavailable | A `409 "integration name already in use"` may actually be a **prefix** collision (the DB unique violation maps both to one error), so check your `route_prefix` too. |
| `422` | Memory store rejected by the gatekeeper (`ErrNotDurable`) | Distinct from validation `400`. |
| `500` | Internal error; malformed `base_url` at proxy time | n/a |
...
- **List envelopes (no pagination):** `GET /integrations` → `{"integrations":[…]}`, `GET /sessions` → `{"sessions":[…]}` are **full, unbounded** lists. `GET /sessions/{id}/history?limit=N` takes a limit but no offset/cursor. Only `/integrations/_calls` is paginated (`cursor`/`limit`).
...
```
> Best practice for unattended bots: instead of `session.idle`, give the agent a `reply_to_user` MCP tool (in `mcp_servers`) and force it via the system prompt; the tool calls back into your service directly, with tighter latency and no idle-threshold tuning.
...
- [ ] Test with Claude AND at least one other provider to verify MCP + system prompt translate.
...
| DO | DON'T |
|---|---|
| ✅ Declare MCP / system prompt / `permission_mode` on the integration row. | ❌ Hand-build `--mcp-config` / `--append-system-prompt` / `--rules` / `--dangerously-skip-permissions` / `--yolo` into `POST /sessions` `args`. |
...
- **Reserved for the future:** the `agent_id` field ([§3](#3-registration--the-api-key)) is accepted and stored today but not read at runtime; the eventual home for a named, reusable agent bundle. Leave it empty.
