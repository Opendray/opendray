...
- **Idempotent**: repeat exit-detector wakeups, a double `Stop`, or a
  ...
  exit) means a user stop is **never** overwritten by a later shutdown. This
  reproduces the historical `classifyExitState` precedence, which
...
Note (per §2): `disconnected` already behaves exactly like this today: the
process is never touched on a WS drop and re-attach replays the ring. The
`orphaned` "adopt probe" is **not pursued** (PTY master fd dies with the
