...
<p align="center">
  <strong>自托管网关，统一接入 Claude Code · Codex · Antigravity · shell，提供跨 CLI 共享的本地优先记忆层。</strong>
  <br/>
...
在日常使用 AI 编码 CLI 的过程中，有三个痛点，opendray 正是为解决它们而生。

**笔记本一休眠，会话就断了。** 通过 SSH 运行 Claude Code 或 Codex，意味着只要合上盖子或 Wi-Fi 一掉，agent 就会立刻死掉。上下文、正在执行的工具调用、那个你正打算审阅的半截 diff，全没了。opendray 把 agent 跑在一台不会休眠的主机上（桌下的 Mac mini、NAS、VPS 都行），让你能从 Web 管理后台、Flutter 移动端 app 或一条聊天消息重新接入。不管有没有人连着，会话都在继续跑。

**撞到速率限制，不该让你手头的活儿一起完蛋。** 如果你手上有多个 Anthropic 账号（工作 + 个人、家庭计划 + Pro），opendray 会把它们当作一个池子来管理：展示每个账号的档位、配额和活跃会话数，在账号之间均衡分配新会话，还允许你把一个进行中的会话切换到另一个账号，对话内容不会丢。会话记录跟着你走。Codex 账号同理。

**记忆是一等公民，不是事后补丁。** 大多数 AI CLI 每次会话都从零开始重新索引项目上下文，把 token 浪费在重复检索上。opendray 自带一个本地优先的向量存储（支持 ONNX / Ollama / LM Studio 嵌入），具备三域检索（用户、项目、会话），并能跨层检测漂移。每一个字节都留在你自己的网络里。

---

## opendray 是什么?

**opendray** 把你已经在用的 AI 编程 CLI(Claude Code、Codex、Antigravity,加上任意 shell)包起来,变成一个你能随处操控的东西。在家用服务器 / NAS / VPS 上跑会话,空闲时 Telegram 给你推送,在手机上回一句话就能把下一个 prompt 送回去。整个网关你自己托管、端到端掌控。
...
引导你完成 Postgres 设置、AI CLI 安装、admin 凭据、服务注册,5 到 10 分钟拉起一个运行中的网关。详见 [**`scripts/README.md`**](scripts/README.md):wizard 做什么、生成的文件布局、参数、排错。

> **想自己一步步来?** 看 [**docs/getting-started.zh.md**](docs/getting-started.zh.md)，这是 15 分钟端到端 walkthrough,跟 wizard 做的是同样的事,但每一步你都自己确认。
...
```sh
sudo opendray providers update   # 升级 AI CLIs(claude / codex / antigravity)到 npm 最新版
```
...
所有支持的方案都包含 session spawn、AI CLI、加密备份、完整集成 API。opendray 是 host-resident 网关，通过 PTY spawn AI CLI、跟它们共享进程状态(`~/.claude`、ssh-agent、项目文件),这跟生产级 Docker 的容器隔离模型不兼容,因此 v2.x 不支持 Docker 部署。
...
```
cmd/opendray/   二进制入口
internal/       Go 后端 —— 网关、会话、记忆、Channel、Integration、Git、搜索…(每个领域一个包)
app/web/        React + Vite 后台 SPA(已 embed 进二进制)
app/mobile/     Flutter app(iOS + Android)
app/shared*/    跨 surface 共享的 UI + i18n 文案
docs/           指南:安装、快速上手、集成、运维
deploy/         systemd / launchd / LXC 单元 + 安装脚本
```
