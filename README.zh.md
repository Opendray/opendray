...
<p align="center">
  <strong>自托管网关，统一接入 Claude Code · Codex · Gemini · shell，提供跨 CLI 共享的本地优先记忆层。</strong>
  <br/>
...
**撞到速率限制，不该让你手头的活儿一起完蛋。** 如果你手上有多个 Anthropic 账号（工作 + 个人、家庭计划 + Pro），opendray 会把它们当作一个池子来管理：展示每个账号的档位、配额和活跃会话数，在账号之间均衡分配新会话，还允许你把一个进行中的会话切换到另一个账号，对话内容不会丢。会话记录跟着你走。Codex 和 Gemini 账号同理。
