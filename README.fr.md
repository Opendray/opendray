...
<p align="center">
  <strong>Gateway self-hosted pour Claude Code · Codex · Antigravity · shell, avec une couche de mémoire local-first partagée entre tous.</strong>
  <br/>
...
Un seul binaire Go sur ton host orchestre l'ensemble. Les clients pilotent les sessions via HTTP/WebSocket, le session manager lance chaque AI CLI dans son propre PTY, et la couche memory conserve l'état partagé dans Postgres avec des vector embeddings issus de ton propre provider.
...
Tout ce qui apparaît dans le diagramme tourne sur ton réseau. Aucune dépendance cloud, aucune inference hors de ton contrôle.

---

## État

**v2.7.6** (dernière). La génération v2 continue d'itérer. Voir
...
|---|---|---|
| 📦 **Binaire pré-construit** | « Just run it » sur Linux / macOS, n'importe quel superviseur | [Page des releases](https://github.com/Opendray/opendray/releases) → voir [Déploiement en production](#production-deploy) |
...
- [`docs/mobile-app.fr.md`](docs/mobile-app.fr.md) : compiler & installer l'app mobile Flutter — sideloader un APK Android ou l'installer sur iPhone via Xcode, puis la pointer vers ton gateway
